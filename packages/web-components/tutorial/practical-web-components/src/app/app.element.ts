  ...
  connectedCallback() {
    this.innerHTML = `
      ...
      <template id='modal-template'>
        <link rel='stylesheet' href='/assets/blog-modal.css' />
    ...
    titles.forEach(title => {
      const node = document.createElement('blog-card')
    ...
    const cards = document.getElementsByTagName('blog-card');
    Array.from(cards).forEach(card => {
      card.addEventListener('click', () => {
        const currentBlog = titles.filter(title => title.title === card.getAttribute('title'));
...
customElements.define('tutorial-blog', AppElement);
