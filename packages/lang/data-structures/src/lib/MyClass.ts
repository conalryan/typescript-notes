...
export class MyClass {
  ...
  myNumber: number;
  myBoolean: boolean;
  noSemiColon: any = 0.25; // semi-colons are optional in TypeScript
  typeInfered = 0.22; // even though not declared as number, compiler knows it's a number.
   ...
   */
  constructor(
    public myNewPublicField: string,
    protected myNewProtectedField: number,
    private myNewPrivateField: boolean
  ) {
   ...
   */
  myBracketlessArrowFunction = (): string =>
    'must return something when no brackets used';
}
