...
export class ChildComponent
  implements
    ...
    AfterViewChecked {
  @Input()
  childInputBindingUsedInTemplate: any;
  @Input()
  childInputBindingNotUsedInTemplate: any;
  @Output()
  childOutputEmitter = new EventEmitter<string>();

  sharedServiceProp: any;
  sharedParentToChildProp: any;
  ...
  ngOnInit(): void {
    ...
    this.httpExService.getUsers().subscribe(res => {
      ...
      this._sharedService.sharedProp = (res as any).data[0]["first_name"];
    });

    this.httpExService.getUsers().subscribe(res => {
      console.warn(res);
      // console.log(res["data"][1]["first_name"]);
      this._sharedService.sharedProp = (res as any).data[1]["first_name"];
    });
